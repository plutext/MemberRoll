...
# /opt/memberroll and rebuilds/restarts the tomcat service. On a
...
rsync -av -e "ssh ${SSH_OPTS:-}" --exclude data/ --exclude backups/ --exclude .env --exclude keycloak-import/ \
    ./ "$HOST:$DIR/"
rsync -av -e "ssh ${SSH_OPTS:-}" ../keycloak/memberroll-realm.json "$HOST:$DIR/realm-src.json"
