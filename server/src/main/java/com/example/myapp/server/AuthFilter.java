...
public class AuthFilter implements ContainerRequestFilter {
     ...
     * {@code http://localhost:8081/realms/myapp,http://192.168.1.50:8081/realms/myapp}.
    ...
    static final List<String> ISSUERS = List.of(
            env("KEYCLOAK_ISSUER", "http://localhost:8081/realms/myapp")
                    .split("\\s*,\\s*"));
    static final String AUDIENCE = env("KEYCLOAK_AUDIENCE", "myapp-server");
    ...
    private static void abort(ContainerRequestContext request, String reason) {
        request.abortWith(Response.status(Response.Status.UNAUTHORIZED)
                .header(HttpHeaders.WWW_AUTHENTICATE, "Bearer realm=\"myapp\"")
