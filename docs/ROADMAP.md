...
| 014 | SMTP settings page (out-of-band) | Proposed | Admin page to configure the outbound relay — Exchange Online (`smtp.office365.com`) as the target with a form-filling preset, general SMTP underneath. One `smtp_settings` JSON row in `app_setting` (no migration); resolution PAGE → ENV → disabled, env demoted to dev/bootstrap fallback; test-send endpoint surfaces the relay's real error. Basic auth is the right v1 (Microsoft keeps it through Dec 2026); XOAUTH2 recorded as the follow-up. Independent of 007/008 |
