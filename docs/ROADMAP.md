...
| 013 | Committee register (out-of-band) | Implemented (2026-07-19, verified) | Records office-bearers + ordinary members as term-bounded appointments (clauses 14–17): current committee, AGM roll (close-all-open-new, atomic like rollover), mid-year casual vacancies, edit/remove corrections, and a `/contacts` seam (current committee emails + the secretary) for CR-007. V7 migration; no Keycloak role (deferred — governance fact lives in the register). Feeds CR-007's committee-approval notification; sibling of 011's compliance work |
| 014 | SMTP settings page (out-of-band) | Implemented (2026-07-19, verified) | Admin page to configure the outbound relay — Exchange Online (`smtp.office365.com`) as the target with a form-filling preset, general SMTP underneath. One `smtp_settings` JSON row in `app_setting` (no migration); `Mail.resolve()` per send PAGE → ENV → disabled, env demoted to dev/bootstrap fallback (ENV path kept byte-compatible with CR-004); test-send endpoint surfaces the relay's real error with the password scrubbed. Matrix +50 `CR14-*` rows green + re-runnable (whole suite 537/3, the 3 pre-existing flakes), Playwright 12/12. Basic auth is the right v1 (Microsoft keeps it through Dec 2026); XOAUTH2 recorded as the follow-up. Independent of 007/008 |
| 017 | Membership card (out-of-band) | Implemented + verified (2026-07-20) | On-demand membership card — shown/downloaded/printed from the self-serve page, or emailed as a PNG attachment (member and admin surfaces, one shared Java2D renderer `Cards`, CR-012 pattern). MEMBER-relationship people only, ACTIVE memberships only (`Cards.compose` reads live household composition, not the snapshot), validity date printed so saved copies expire visibly; wallet passes rejected (two vendor integrations + annual cert renewal). Brings `Mail` attachment support (no-attachment path proven byte-for-byte unchanged). No schema/realm change. Verified: full matrix 649/0 (+46 self-cleaning `CR17-*` rows, re-run identical), Playwright 15/15, PNG confirmed legible at print size. See [017-membership-card.md](change-requests/017-membership-card.md) |

| 021 | Mail sandbox redirect (out-of-band) | Proposed (2026-07-23) | Testing safety for the demo instance (real member data + live Exchange): one optional `redirectTo` field in the CR-014 `smtp_settings` blob diverts EVERY outgoing message (guest-triggered sends included) to a single sandbox address, original recipient marked in subject and body, real SMTP path still exercised. Ambient banner on every admin page while active; blank = live, byte-for-byte unchanged. Keycloak's own forgot-password mail is realm config and needs the paired manual step (runbook). ENV backstop (the "C hybrid") deferred to a possible follow-up. See [021-mail-sandbox-redirect.md](change-requests/021-mail-sandbox-redirect.md) |
