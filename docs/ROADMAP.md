...
| 017 | Membership card (out-of-band) | Proposed (2026-07-20) | On-demand membership card — shown/downloaded/printed from the self-serve page, or emailed as a PNG attachment (member and admin surfaces, one shared Java2D renderer, CR-012 pattern). MEMBER-relationship people only, ACTIVE memberships only, validity date printed so saved copies expire visibly; wallet passes rejected (two vendor integrations + annual cert renewal). Brings `Mail` attachment support. No schema/realm change; independent of the sequence |
