...
commit), and removes itself. Everything below assumes the id `myapp` —
yours will differ.
...
TOKEN=$(curl -s -X POST http://localhost:8081/realms/myapp/protocol/openid-connect/token \
...
- <http://localhost:8081/> — the Keycloak admin console
  ...
  `server/keycloak/myapp-realm.json` — a `docker compose down` discards
...
`server/keycloak/myapp-realm.json`. Never wipe the production Keycloak
